import os
import string
import math
import base64
import re
import random
from flask import Flask, request, Blueprint, Response, current_app, send_file, jsonify, send_from_directory
import requests
import jsonpickle
from api.common import *
from model.origin import *
from oauth import *

api_origins = Blueprint("api_origins", __name__)
oauth = get_oauth_provider()

@api_origins.route("/origins", methods=["GET"])
def api_origins_get():
    sort_by, results_page, results_page_size, filter_by = get_select_params(request)

    results, result_count = Origin.get_paged(
        sort_by=sort_by,
        results_page=results_page,
        results_page_size=results_page_size,
        filter_by=filter_by)

    total_pages = math.ceil(result_count / results_page_size)
    json_str = jsonpickle.encode({"current_page": results_page, "page_size": results_page_size, "total_count": result_count,
                                  "total_pages": total_pages, "sort_by": sort_by, "results": results}, unpicklable=False)

    return Response(response=json_str, status=200, mimetype="application/json")

@api_origins.route("/origins/<int:id_origin>", methods=["GET"])
def api_origins_id_get(id_origin):
    try:
        result = Origin(id_origin=id_origin)
    except:
        return Response("not found", 404)

    return Response(jsonpickle.encode(result, unpicklable=False), status=200, mimetype="application/json")

@api_origins.route("/origins", methods=["POST"])
@oauth.require_oauth()
def api_origins_post():
    is_json_ok, data, json_error = parse_json(request.data, [
            "name",
            "date_changed"])

    if not is_json_ok:
        return Response(json_error, 400)

    if not (request.oauth.user.is_admin) and not (request.oauth.user.id_user == data["id_user"]):
        return Response("unauthorized user", 401)

    origin = Origin()
    origin.name = data["name"]
    origin.date_changed = data["date_changed"]

    origin.insert()
    origin.reload()

    json_str = jsonpickle.encode(origin, unpicklable=False)

    return Response(response=json_str, status=201, mimetype="application/json")

@api_origins.route("/origins/<int:id_origin>", methods=["PUT"])
@oauth.require_oauth()
def api_origins_id_put(id_origin):
    is_json_ok, data, json_error = parse_json(request.data, [
            "name",
            "date_changed"])

    if not is_json_ok:
        return Response(json_error, 400)

    try:
        origin = Origin(id_origin = id_origin)
    except:
        return Response("not found", 404)

    if not (request.oauth.user.is_admin) and not (request.oauth.user.id_user == data["id_user"] and request.oauth.user.id_user == origin.id_user):
        return Response("unauthorized user", 401)

    origin.name = data["name"]
    origin.date_changed = data["date_changed"]

    origin.update()
    origin.reload()

    json_str = jsonpickle.encode(origin, unpicklable=False)

    return Response(response=json_str, status=200, mimetype="application/json")

@api_origins.route("/origins/<int:id_origin>", methods=["DELETE"])
@oauth.require_oauth()
def api_origins_id_delete(id_origin):
    try:
        origin = Origin(id_origin=id_origin)
    except:
        return Response("not found", 404)

    if not (request.oauth.user.is_admin) and not (request.oauth.user.id_user == origin.id_user):
        return Response("unauthorized user", 401)        

    origin.delete()

    return Response("deleted", status=200)    
