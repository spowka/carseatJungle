...
class ChildHeightGroup(object):

    def __init__(self, id_child_height_group=None, db_row=None):
        if db_row != None:
            self.fill_from_data_row(db_row)
        elif id_child_height_group != None:
            rows, _ = self.db_select(id_child_height_group=id_child_height_group)
            if len(rows) == 0:
    ...
    def insert(self):
        self.id_child_height_group = pg_execute("\
        insert into child_height_group(\
            name,\
            date_changed)\
        values(%s,%s) returning id_child_height_group",
        (
            self.name,
            self.date_changed
        ))
    ...
    def reload(self):
        rows, _ = self.db_select(id_child_height_group=self.id_child_height_group)
        self.fill_from_data_row(rows[0])           

    @staticmethod
    def db_select(id_child_height_group=None,
                  sort_by=None,
                  results_page=None,
        ...
        return pg_select_rows_with_count("\
            ...
            from\
                child_height_group\
            where\
                (%(id_child_height_group)s is null or id_child_height_group = %(id_child_height_group)s)\
           {0} {1} {2}".format(filter_string, order_string, limit_string),
            {
                "id_child_height_group": id_child_height_group
            })

    @staticmethod
    def get_all():
        items = []
        rows, _ = ChildHeightGroup.db_select()
        for row in rows:
            item = ChildHeightGroup()
    ...
    @staticmethod
    def get_paged(id_child_height_group=None,
                  sort_by=None,
                  results_page=None,
                  results_page_size=None,
                  filter_by=None):
        ...
        rows, row_count = ChildHeightGroup.db_select(
            id_child_height_group=id_child_height_group,
            sort_by=sort_by,
